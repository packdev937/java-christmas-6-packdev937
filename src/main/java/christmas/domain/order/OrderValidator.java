...
public class OrderValidator {
    ...
    private static final int MINIMUM_ORDER_CRITERION = 1;
    private static final int MAXIMUM_ORDER_CRITERION = 20;
    ...
    private static void validateMinimumOrderQuantity(Map<MenuItem, Integer> items) {
        ...
        if (totalQuantity < MINIMUM_ORDER_CRITERION) {
    ...
    private static void validateMaximumItems(Map<MenuItem, Integer> items) {
        int totalItems = items.values().stream()
            ...
            .sum();
        if (totalItems > MAXIMUM_ORDER_CRITERION) {
            throw new IllegalArgumentException(
                ORDER_ERROR_PREFIX + "메뉴는 한 번에 최대 " + MAXIMUM_ORDER_CRITERION
                    + "개까지만 주문할 수 있습니다.");
        }
    }
}
